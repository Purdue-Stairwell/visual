class Creature {
	constructor(colorVar, agitatedness, speed, size, sprite, x, y) {
		// chosen  by user
		this.color = colorVar;
		...
		this.x = Array(this.points.length).fill(this.pos.x);
	...
	drawCreatures() {
		...
		// draw sprites
		image(tails[this.sprite], this.x[this.points.length], this.y[this.points.length], 60 * this.size, 60 * this.size);

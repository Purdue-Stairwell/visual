	...
	drawCreatures() {
		...
		push();
			...
			for (let i = 0; i < this.points.length; i++) {
				noFill();
				stroke(this.color);
				vertex(lerp(this.points[i]["x"], this.x[i], 0.5), lerp(this.points[i]["y"], this.y[i], 0.5));
			}
			endShape();
			...
			/* for (let i = 0; i < (this.points.length - 1); i += 2) {
				...
				image(bodies[this.sprite][this.colorIndex], (this.x[i] + this.points[i]["x"]), (this.y[i] + this.points[i]["y"]), 40 * this.size, 40 * this.size);
			}  */
		pop();
	}
}
